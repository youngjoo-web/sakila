...
<html>
	...
	<%
		...
		InventoryDao inventoryDao = new InventoryDao();
     	ArrayList<InventoryAndFilmAndStore> list = inventoryDao.selectInventoryAndFilmAndStorelistAll(beginRow, rowPerPage);
    	
     	// 페이징 관련 로직 (lastPage, totalCount)
		int totalCount = 0;
		totalCount = inventoryDao.selectTotalCount(totalCount);
	    	...
	    	<div id="main">
		        ...
		        <div>
					<table class="table table-striped">
						...
						<tbody>
						<%
							for(InventoryAndFilmAndStore ln: list){
						%>
							<tr>
								<td><%=ln.getInventory().getInventoryId()%></td>
								<td><%=ln.getFilm().getFilmId()%></td>
								<td><%=ln.getFilm().getTitle() %></td>
								<td><%=ln.getStore().getStoreId()%></td>
								<td><%=ln.getFilm().getLastUpdate()%></td>
							</tr>
						<%
							}
						%>
